...
void hysteria2Construct(Proxy &node, const std::string &group, const std::string &remarks, const std::string &server, const std::string &port, const std::string &ports, const std::string &up, const std::string &down, const std::string &password, const std::string &obfs, const std::string &obfs_password, const std::string &sni, const std::string &fingerprint, const std::string &alpn, const std::string &ca, const std::string &ca_str, const std::string &cwnd, const std::string &hop_interval, const std::string &ech_enable, const std::string &ech_config, const std::string &initial_stream_receive_window, const std::string &max_stream_receive_window, const std::string &initial_connection_receive_window, const std::string &max_connection_receive_window, tribool udp, tribool tfo, tribool scv, const std::string &underlying_proxy) {
...
void vlessConstruct(Proxy &node, const std::string &group, const std::string &remarks, const std::string &add, const std::string &port, const std::string &type, const std::string &uuid, const std::string &aid, const std::string &net, const std::string &cipher, const std::string &flow, const std::string &mode, const std::string &path, const std::string &host, const std::string &edge, const std::string &tls, const std::string &public_key, const std::string &sid, const std::string &fingerprint, const std::string &sni, const std::vector<std::string> &alpnList, const std::string &packet_encoding, tribool udp, tribool tfo, tribool scv, tribool tls13, const std::string &underlying_proxy, tribool v2ray_http_upgrade, tribool v2ray_http_upgrade_fast_open)
{
    commonConstruct(node, ProxyType::VLESS, group, remarks, add, port, udp, tfo, scv, tls13, underlying_proxy);
    node.UserId = uuid.empty() ? "00000000-0000-0000-0000-000000000000" : uuid;
    node.AlterId = to_int(aid);
    node.EncryptMethod = cipher;
...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    std::string sid, packet_encoding, v2ray_http_upgrade, v2ray_http_upgrade_fast_open; // vless
    string_array dns_server;
    tribool udp, tfo, scv, udp_over_tcp;
    tribool v2ray_http_upgrade_tribool, v2ray_http_upgrade_fast_open_tribool;
    std::vector<std::string> alpnList;
    Node singleproxy;
    uint32_t index = nodes.size();
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
            ...
            case "ws"_hash:
                if(singleproxy["ws-opts"].IsDefined())
                {
                    ...
                    if(singleproxy["ws-opts"]["v2ray-http-upgrade-fast-open"].IsDefined())
                        v2ray_http_upgrade_fast_open = safe_as<std::string>(singleproxy["ws-opts"]["v2ray-http-upgrade-fast-open"]);
                }
                else{
        ...
        case "ss"_hash:
            ...
            if(singleproxy["plugin"].IsDefined() || singleproxy["obfs"].IsDefined())
            {
                if(singleproxy["plugin"].IsDefined())
                {
                    std::string plugin_type = safe_as<std::string>(singleproxy["plugin"]);
                    switch(hash_(plugin_type))
                    {
                    case "obfs"_hash:
                        plugin = "obfs-local";
        ...
        case "trojan"_hash:
            ...
            case "ws"_hash:
                if(singleproxy["ws-opts"].IsDefined())
                {
                    ...
                    {
                        // 只取 Host，其他 header 可扩展
                        // 这里可扩展为 map<string, string> 存 Proxy.WsHeaders
                    }
                }
                break;
            default:
        ...
        case "vless"_hash:
        {
            ...
            if (!v2ray_http_upgrade.empty()) {
                v2ray_http_upgrade_tribool = v2ray_http_upgrade == "true";
            }
            if (!v2ray_http_upgrade_fast_open.empty()) {
                v2ray_http_upgrade_fast_open_tribool = v2ray_http_upgrade_fast_open == "true";
            }
            vlessConstruct(node, VLESS_DEFAULT_GROUP, ps, server, port, type, id, aid, net, "auto", flow, mode, path, host, "", tls, public_key, sid, fingerprint, sni, alpnList, packet_encoding, vless_udp, tfo, scv, tribool(), "", v2ray_http_upgrade_tribool, v2ray_http_upgrade_fast_open_tribool);
            break;
        }
        case "tuic"_hash:
...
void explodeStdHysteria(std::string hysteria, Proxy &node)
{
    ...
    hysteriaConstruct(node, HYSTERIA_DEFAULT_GROUP, remarks, add, port, ports, protocol, obfs_protocol, up, up_speed, down, down_speed, auth, auth_str, obfs, sni, fingerprint, ca, ca_str, recv_window_conn, recv_window, disable_mtu_discovery, hop_interval, alpn, tribool(), scv, "");
    return;
}

void explodeStdVLESS(std::string vless, Proxy &node)
{
    std::string add, port, type, uuid, aid, net, flow, public_key, sid, fingerprint, mode, path, host, tls, remarks, sni, xtls;
    std::string addition;
    std::string alpn;
    std::vector<std::string> alpnList;
    tribool tfo, scv;
    ...
    if(regGetMatch(vless, stdvless_matcher, 5, 0, &uuid, &add, &port, &addition))
        return;
    ...
    std::string xudp_param = getUrlArg(addition, "xudp");
    if(!xudp_param.empty())
    {
        node.XUDP = tribool(xudp_param);
    }
    ...
    std::string ws_max_early_data = getUrlArg(addition, "max-early-data");
    if(!ws_max_early_data.empty())
    {
        node.WsMaxEarlyData = to_int(ws_max_early_data);
    }
    std::string ws_early_data_header_name = getUrlArg(addition, "early-data-header-name");
    ...
    scv = tribool(getUrlArg(addition, "skip-cert-verify"));
    ...
    vlessConstruct(node, VLESS_DEFAULT_GROUP, remarks, add, port, type, uuid, aid, net, "auto", flow, mode, path, host, "", tls, public_key, sid, fingerprint, sni, alpnList, packet_encoding, tribool(), tfo, scv);
    return;
}
...
void explodeStdTUIC(std::string TUIC, Proxy &node)
{
    ...
    if (remarks.empty())
        remarks = add + ":" + port;
    TUICConstruct(node, TUIC_DEFAULT_GROUP, remarks, add, port, uuid, password, ip, heartbeat_interval, alpn, disable_sni, reduce_rtt, request_timeout, udp_relay_mode, congestion_controller, max_udp_relay_packet_size, max_open_streams, sni, fast_open, tfo, scv, "");
}
        ...
        */
        ...
        default:
            ...
            case "vless"_hash: // quantumult x style vless link
                ...
                if(remarks.empty())
                    remarks = server + ":" + port;
                vlessConstruct(node, VLESS_DEFAULT_GROUP, remarks, server, port, type, id, aead, net, method, "", "", path, host, "", tls, "", "", fingerprint, sni, std::vector<std::string>{}, "", udp, tfo, scv, tls13);
                break;
            default:
                continue;
            }
...
void explodeSingbox(rapidjson::Value &outbounds, std::vector<Proxy> &nodes)
{
    ...
    for(rapidjson::SizeType i = 0; i < outbounds.Size(); ++i)
    {
        if(outbounds[i].IsObject())
        {
            ...
            std::string sid, packet_encoding; // vless
            std::string idle_session_check_interval, idle_session_timeout, min_idle_session; // anyTLS
            string_array dns_server;
            ...
            if(singboxNode.HasMember("type") && singboxNode["type"].IsString())
            {
                ...
                switch(hash_(proxytype))
                {
                    ...
                    case "vless"_hash:
                        ...
                        vlessConstruct(node, group, ps, server, port, type, id, aid, net, "auto", flow, mode, path, host, "", tls, public_key, sid, fingerprint, sni, alpnList, packet_encoding, udp);
                        break;
                    case "tuic"_hash:
                        group = TUIC_DEFAULT_GROUP;
