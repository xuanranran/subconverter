...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    for (uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "vless"_hash:
            ...
            vlessConstruct(node, XRAY_DEFAULT_GROUP, ps, server, port, type, id, aid, net, "auto", flow, mode, path, host, "", tls, pbk, sid, fp, sni, alpnList, packet_encoding, udp);
            break;
        case "hysteria"_hash:
            ...
            hysteriaConstruct(node, group, ps, server, port, type, auth, "", host, up, down, alpn, obfsParam, insecure, ports, sni, udp, tfo, scv);
            break;
        case "hysteria2"_hash:
            ...
            hysteria2Construct(node, group, ps, server, port, password, host, up, down, alpn, obfsParam, obfsPassword, sni, public_key, ports, udp, tfo, scv);
            break;
        case "tuic"_hash:
            ...
            tuicConstruct(node, TUIC_DEFAULT_GROUP, ps, server, port, password, congestion_control, alpn, sni, id, udp_relay_mode, token, tribool(), tribool(), scv, reduceRtt, disableSni, request_timeout);
...
void explodeStdHysteria(std::string hysteria, Proxy &node)
{
    ...
    hysteriaConstruct(node, HYSTERIA_DEFAULT_GROUP, remarks, add, port, type, auth, auth_str, host, up, down, alpn, obfsParam, insecure, "", sni);
    return;
}
...
void explodeStdHysteria2(std::string hysteria2, Proxy &node)
{
    ...
    hysteria2Construct(node, HYSTERIA2_DEFAULT_GROUP, remarks, add, port, password, host, up, down, alpn, obfsParam, obfsPassword, host, "", ports, tribool(), tribool(), scv);
    return;
}
        ...
        if (outbounds[i].IsObject())
        {
            ...
            if (singboxNode.HasMember("type") && singboxNode["type"].IsString())
            {
                ...
                case "hysteria2"_hash:
                    ...
                    hysteria2Construct(node, group, ps, server, port, password, host, up, down, alpn, obfsParam, obfsPassword, sni, public_key, "", udp, tfo, scv);
                    break;
                case "tuic"_hash:
                    group = TUIC_DEFAULT_GROUP;
                    password = GetMember(singboxNode, "password");
                    ...
                    tuicConstruct(node, TUIC_DEFAULT_GROUP, ps, server, port, password, congestion_control, alpn, sni, id, udp_relay_mode, "", tribool(), tribool(), scv, rrt, disableSni);
                    break;
                default:
                    continue;
                }
...
void explodeTuic(const std::string &tuic, Proxy &node)
{
    ...
    if (remarks.empty())
        remarks = add + ":" + port;
    tuicConstruct(node, TUIC_DEFAULT_GROUP, remarks, add, port, password, congestion_control, alpn, sni, uuid, "native", "", tribool(), tribool(), scv);
...
void explode(const std::string &link, Proxy &node)
{
    ...
    else if (startsWith(link, "hysteria://") || startsWith(link, "hy://"))
